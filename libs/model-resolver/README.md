...
```bash
npm install @aboardai/model-resolver
```
...
import { CLAUDE_MODEL_MAP } from '@aboardai/types';
...
```typescript
import { getEffectiveModel } from '@aboardai/model-resolver';
...
```typescript
import { DEFAULT_MODELS } from '@aboardai/model-resolver';
import { CLAUDE_MODEL_MAP } from '@aboardai/types';
...
- `@aboardai/types` - Model type definitions and constants

## Used By

- `@aboardai/server` - Feature execution, agent chat, enhancement
