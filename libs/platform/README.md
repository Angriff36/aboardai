...
This package provides platform-specific utilities including path management, subprocess handling, and security validation. It handles AboardAI's directory structure and system operations.

## Installation

```bash
npm install @aboardai/platform
```
...
AboardAI directory structure utilities.
...
// Get AboardAI directory: /project/.aboardai
const aboardaiDir = getAboardAIDir('/project/path');

// Get features directory: /project/.aboardai/features
const featuresDir = getFeaturesDir('/project/path');

// Get specific feature directory: /project/.aboardai/features/feature-id
const featureDir = getFeatureDir('/project/path', 'feature-id');

// Get feature images: /project/.aboardai/features/feature-id/images
const imagesDir = getFeatureImagesDir('/project/path', 'feature-id');

// Ensure .aboardai directory exists
await ensureAboardAIDir('/project/path');
```
...
```typescript
import { spawnJSONLProcess, spawnProcess } from '@aboardai/platform';
...
async function executeFeature(projectPath: string, featureId: string) {
  ...
  // Ensure AboardAI directory exists
  await ensureAboardAIDir(safePath);
...
AboardAI uses the following directory structure:
...
- `@aboardai/types` - Type definitions
