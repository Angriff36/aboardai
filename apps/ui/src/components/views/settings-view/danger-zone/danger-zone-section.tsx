...
interface DangerZoneSectionProps {
  ...
  onDeleteClick: () => void;
  onRemoveFromAboardAIClick?: () => void;
}

export function DangerZoneSection({
  project,
  onDeleteClick,
  onRemoveFromAboardAIClick,
            ...
            {onRemoveFromAboardAIClick && (
              <div className="flex items-start justify-between gap-4 p-4 rounded-xl bg-muted/30 border border-border">
                <div className="min-w-0">
                  <p className="font-medium text-foreground">Remove from AboardAI</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Remove this project from AboardAI without deleting any files from disk. You can
                    re-add it later by opening the folder.
                  </p>
                </div>
                <Button
                  variant="secondary"
                  onClick={onRemoveFromAboardAIClick}
