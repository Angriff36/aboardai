...
for (const pkgName of LOCAL_PACKAGES) {
  const pkgDir = pkgName.replace('@aboardai/', '');
...
const nodeModulesAboardAI = join(BUNDLE_DIR, 'node_modules', '@aboardai');
for (const pkgName of LOCAL_PACKAGES) {
  const pkgDir = pkgName.replace('@aboardai/', '');
  const nmPkgPath = join(nodeModulesAboardAI, pkgDir);
  try {
