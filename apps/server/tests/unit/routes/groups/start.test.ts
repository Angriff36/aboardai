/**
 * Unit tests for POST /api/groups/start handler
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import type { Request, Response } from 'express';
import { createMockExpressContext } from '../../../utils/mocks.js';
import { createStartHandler } from '../../../../src/routes/groups/routes/start.js';
...
describe('POST /api/groups/start', () => {
  let req: Request;
  let res: Response;
  ...
  function buildGroupService(
    ...
    const mockQueue = {
      startGroup: opts.queueStartThrows
        ? vi.fn().mockRejectedValue(opts.queueStartThrows)
  ...
  it('happy path: queue.startGroup called, returns snapshot', async () => {
    const { mockGroupService, mockQueue } = buildGroupService();
    req.body = { projectPath: '/test/proj', groupId: 'group-abc' };

    const handler = createStartHandler(mockGroupService);
    await handler(req, res);

    expect(mockQueue.startGroup).toHaveBeenCalledWith('/test/proj', 'group-abc');
  ...
  it('409: GroupCommandDenied when starting a running group', async () => {
    const denial = new GroupCommandDenied('startGroup denied: status is not pending', {});
    const { mockGroupService } = buildGroupService({ queueStartThrows: denial });
    req.body = { projectPath: '/test/proj', groupId: 'group-abc' };
