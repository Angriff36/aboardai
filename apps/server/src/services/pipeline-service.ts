 ...
 * - Pipeline configuration ({projectPath}/.aboardai/pipeline.json)
 */
...
import type { PipelineConfig, PipelineStep, FeatureStatusWithPipeline } from '@aboardai/types';
...
function getPipelineConfigPath(projectPath: string): string {
  return path.join(projectPath, '.aboardai', 'pipeline.json');
}
