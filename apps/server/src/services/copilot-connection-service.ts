...
function getMarkerPath(projectRoot?: string): string {
  ...
  const aboardaiDir = path.join(root, '.aboardai');
  return path.join(aboardaiDir, COPILOT_DISCONNECTED_MARKER_FILE);
}
...
export async function disconnectCopilot(projectRoot?: string): Promise<void> {
  const root = projectRoot || process.cwd();
  const aboardaiDir = path.join(root, '.aboardai');
  const markerPath = path.join(aboardaiDir, COPILOT_DISCONNECTED_MARKER_FILE);
