...
import { findCliInWsl, isWslAvailable } from '@aboardai/platform';
import {
  CliProvider,
  type CliSpawnConfig,
...
export class CursorProvider extends CliProvider {
  ...
  /**
   * Convert Cursor event to AboardAI ProviderMessage format
  ...
  async *executeQuery(options: ExecuteOptions): AsyncGenerator<ProviderMessage> {
    ...
    if (options.mcpServers && Object.keys(options.mcpServers).length > 0) {
      ...
      logger.warn(
        `MCP servers configured (${serverCount}) but not yet supported by Cursor CLI in AboardAI. ` +
    ...
    // Debug: log raw events when ABOARDAI_DEBUG_RAW_OUTPUT is enabled
