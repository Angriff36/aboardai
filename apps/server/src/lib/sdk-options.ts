...
import { isPathAllowed, PathNotAllowedError, getAllowedRootDirectory } from '@aboardai/platform';
 ...
 * - ABOARDAI_MODEL_SPEC: Model for spec generation
 * - ABOARDAI_MODEL_FEATURES: Model for feature generation
 * - ABOARDAI_MODEL_SUGGESTIONS: Model for suggestions
 * - ABOARDAI_MODEL_CHAT: Model for chat
 * - ABOARDAI_MODEL_DEFAULT: Fallback model for all operations
 */
export function getModelForUseCase(
  ...
  const envVarMap: Record<string, string | undefined> = {
    spec: process.env.ABOARDAI_MODEL_SPEC,
    features: process.env.ABOARDAI_MODEL_FEATURES,
    suggestions: process.env.ABOARDAI_MODEL_SUGGESTIONS,
    chat: process.env.ABOARDAI_MODEL_CHAT,
    auto: process.env.ABOARDAI_MODEL_AUTO,
    default: process.env.ABOARDAI_MODEL_DEFAULT,
  };
...
// Re-export MCP types from @aboardai/types for convenience
export type {
  McpServerConfig,
...
export function createSpecGenerationOptions(config: CreateSdkOptionsConfig): Options {
  ...
  return {
    ...
    // See: https://github.com/AboardAI-Org/aboardai/issues/149
