 ...
 * Stores worktree-specific data in .aboardai/worktrees/:branch/worktree.json
 */

import * as secureFs from './secure-fs.js';
import * as path from 'path';
import type { PRState, WorktreePRInfo } from '@aboardai/types';
...
function getWorktreeMetadataDir(projectPath: string, branch: string): string {
  ...
  return path.join(projectPath, '.aboardai', 'worktrees', safeBranch);
}
...
export async function readAllWorktreeMetadata(
  ...
  const worktreesDir = path.join(projectPath, '.aboardai', 'worktrees');
