/**
 * POST /api/groups/cancel — Cancel a running task group
 */
...
export function createCancelHandler(groupService: GroupService) {
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Check group exists in engine before cancelling; rehydrate if needed
      try {
      ...
      await queue.cancelGroup(projectPath, groupId);
      ...
      logError(error, 'Cancel group failed');
